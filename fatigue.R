...
fatigue_nnet <- function(dataset, hidden, testset, algorithm="rprop+", threshold = 0.01,
                         formula = FatigueLevel ~ Performance.KDTMean + Performance.MAMean +
  ...
  # Saving RSME
  nnet$rmse <- rmse( c(testset$FatigueLevel), c(nnet$prediction) )
